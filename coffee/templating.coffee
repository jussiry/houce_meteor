...
if Meteor.isClient # TODO: Why is this loaded on sever when package says 'client'?
  ...
  Houce.init_templates = ->
    for templ_name, container of Template
      ...
      container.render = -> Houce.parse_template container.html # HTML.Raw
  ...
  Houce.parse_template = (html_obj_or_func)->
    ...
    Houce.tmpl_iterator html_obj, null, templFragment = document.createDocumentFragment()
  ...
  do ->
    ...
    Houce.tmpl_iterator = iterate = (el, key, parent)->
      return unless el?
      if not key?
        ...
        if el instanceof Array
          for sub_el in el
            (iterate sub_el, null, parent) #, data
        else
          (iterate sub_el, sub_key, parent) for sub_key, sub_el of el # key is el when 1 argument
      else                    # , data
        key = key.split('%')[0]
        ...
        if key[0] is '_'
          # attribute value
          parent.setAttribute key[1..-1], el
        ...
        #   Houce.tmpl_stack.last().push el
          ...
          if typeof el is 'object' then iterate el \ # , data
                                   else debugger #Houce.tmpl_stack.last().push el
        else
          if key.match /^render/
            return
            ...
            switch typeof el #el.constructor
              when 'object'
                #Houce.tmpl_stack.push [k = Houce.create_html_node key]
                parent = Houce.create_html_node key
                if el instanceof Array
                then (iterate sub_el, null, parent) for sub_el in el
                else iterate el, null, parent #, data
                #Houce.tmpl_stack.at(-2).push Houce.combine_tag_arr Houce.tmpl_stack.pop()
              when 'string', 'number'
                parent.appendChild newEl = Houce.create_html_node key
                newEl.appendChild document.createTextNode el
                #Houce.combine_tag_arr [ Houce.create_html_node(key), el ], parent
                #Houce.tmpl_stack.last().push Houce.combine_tag_arr [ Houce.create_html_node(key), el ]
      return
  ...
  Houce.render_spark = (tmpl_or_name)-> # , extra_data
    ...
    res = Houce.render_spark_partial tmpl_or_name
